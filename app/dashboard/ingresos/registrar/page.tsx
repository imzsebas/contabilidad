...
interface Row    { id: string; valores: Record<string, number> }
interface DistItem {
...
function defaultDist(cols: Column[], rows: Row[]): DistItem[] {
  return [
...
function calcDist(items: DistItem[], cols: Column[], rows: Row[]): DistItem[] {
  ...
  return items.map(item => {
    let mc = 0
    if (item.porcentaje === 'especial') {
      mc = Math.round(sinVA * 0.20 + va * 0.10)
    } else {
      mc = Math.round(sinVA * (Number(item.porcentaje) / 100))
    }
...
export default function RegistrarIngreso() {
  ...
  const [rows,    setRows]    = useState<Row[]>([{ id: uid(), valores: {} }])
  const [dist,    setDist]    = useState<DistItem[]>(() => defaultDist(DEFAULT_COLS, []))
  ...
  // recalcula distribución cuando cambian filas/columnas
  ...
  function agregarFila() {
    const next = [...rows, { id: uid(), valores: {} }]
    setRows(next)
  ...
  function agregarConcepto() {
    ...
    const next = calcDist([...dist, item], cols, rows)
    setDist(next)
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        const { data: row, error: e3 } = await supabase
          .from('income_rows').insert({ record_id: rec.id, orden: i }).select().single()
        if (e3) throw e3
        const vals = cols.map(c => ({ row_id: row.id, column_id: colMap[c.id], monto: rows[i].valores[c.id] || 0 }))
  ...
  function generarPDF() {
    ...
    const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>
      *{margin:2px;padding:0;box-sizing:border-box}
      ...
      .firma-section{background:#d4edda;padding:8px;text-align:center;font-size:10px;font-weight:bold}
  ...
  async function generarExcel() {
    ...
    wb.creator = 'Estudio Contable Abaco'
    wb.created = new Date()
    ...
    function dataCell(cell: any, value: any, numFmt?: string, bold = false, align: string = 'left') {
      cell.value = value
    ...
    // ═══════════════════════════════════════════════════
    // HOJA 1 — Comprobante de Ingreso
    // ═══════════════════════════════════════════════════
    const ws1 = wb.addWorksheet('Comprobante de Ingreso')
    ws1.columns = [
      { width: 20 }, { width: 30 }, { width: 20 }, { width: 20 }, { width: 28 }
    ]

    // Fila 1 — Título
    ws1.mergeCells('A1:E1')
    hdrStyle(ws1.getCell('A1'), VERDE_OSC)
    ...
    dataCell(ws1.getCell('A2'), 'Ciudad: Monteria', undefined, false)
    ...
    dataCell(ws1.getCell('A3'), 'Recibido de: Hermanos Iglesia en Monteria', undefined, false)
    ...
    // Fila 4 — Total en letras
    ws1.mergeCells('A4:E4')
    dataCell(ws1.getCell('A4'), 'La suma de (en letras): ' + numeroALetras(general) + ' m/cte', undefined, true)
    ws1.getCell('A4').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: VERDE_CLARO } }
    ws1.getRow(4).height = 18

    // Fila 5 — Por concepto de
    ws1.mergeCells('A5:E5')
    dataCell(ws1.getCell('A5'), 'Por concepto de: Diezmos y ofrendas', undefined, false)

    // Fila 6 — Efectivo
    ws1.mergeCells('A6:E6')
    dataCell(ws1.getCell('A6'), '✓ Efectivo', undefined, true)

    // Fila 7 — Encabezado tabla contable
    ws1.getRow(7).height = 20
    ;['Codigo P.U.C.', 'Cuenta', 'Debitos', 'Creditos', 'Firma y Sello'].forEach((h, i) => {
      hdrStyle(ws1.getCell(7, i + 1), VERDE_CLARO, VERDE_OSC)
      ws1.getCell(7, i + 1).value = h
    })

    // Fila 8 — Caja General débito
    let r = 8
    dataCell(ws1.getCell(r, 1), '1105-05', undefined, false, 'center')
    dataCell(ws1.getCell(r, 2), 'Caja General', undefined, false)
    dataCell(ws1.getCell(r, 3), general, '0,##0', false, 'right')
    dataCell(ws1.getCell(r, 4), '', undefined, false)
    ws1.getCell(r, 5).border = allBorders
    r++

    // Filas créditos por columna
    cols.forEach(c => {
      const t = porCol[c.id] || 0
      if (t > 0) {
        dataCell(ws1.getCell(r, 1), c.codigoPUC || '', undefined, false, 'center')
        dataCell(ws1.getCell(r, 2), c.nombre, undefined, false)
    ...
    ws1.getCell(r, 3).value = general
    ws1.getCell(r, 3).numFmt = '0,##0'
    ws1.getCell(r, 3).alignment = { horizontal: 'right' }
    ws1.getCell(r, 4).value = general
    ws1.getCell(r, 4).numFmt = '0,##0'
    ws1.getCell(r, 4).alignment = { horizontal: 'right' }

    // ═══════════════════════════════════════════════════
    // HOJA 2 — Registro detallado (fila por fila)
    // ═══════════════════════════════════════════════════
    const ws2 = wb.addWorksheet('Ingresos')
    const ws2cols = [{ width: 8 }, ...cols.map(() => ({ width: 22 })), { width: 20 }]
    ws2.columns = ws2cols

    // Fila 1 — Iglesia
    ws2.mergeCells(1, 1, 1, cols.length + 2)
    hdrStyle(ws2.getCell('A1'), VERDE_OSC)
    ws2.getCell('A1').value = 'IGLESIA EN MONTERIA'
    ws2.getCell('A1').font = { bold: true, color: { argb: BLANCO }, size: 13 }
    ws2.getRow(1).height = 26

    // Fila 2 — Subtítulo
    ws2.mergeCells(2, 1, 2, cols.length + 2)
    hdrStyle(ws2.getCell('A2'), VERDE_CLARO, VERDE_OSC)
    ws2.getCell('A2').value = 'REGISTRO DE INGRESOS'
    ws2.getRow(2).height = 20

    // Fila 3 — Fecha
    ws2.mergeCells(3, 1, 3, cols.length + 2)
    ws2.getCell('A3').value = 'Fecha del registro: ' + fechaStr
    ws2.getCell('A3').font = { bold: false, size: 10 }
    ws2.getCell('A3').border = allBorders
    ws2.getRow(3).height = 16

    // Fila 4 — Encabezados columnas
    ws2.getRow(4).height = 22
    ws2.getCell(4, 1).value = '#'
    hdrStyle(ws2.getCell(4, 1), VERDE_CLARO, VERDE_OSC)
    cols.forEach((c, i) => {
      const cell = ws2.getCell(4, i + 2)
      hdrStyle(cell, VERDE_CLARO, VERDE_OSC)
      cell.value = c.nombre + ' (' + c.codigoPUC + ')'
    })
    const totalHdrCell = ws2.getCell(4, cols.length + 2)
    hdrStyle(totalHdrCell, VERDE_CLARO, VERDE_OSC)
    totalHdrCell.value = 'Total Fila'

    // Filas de datos
    rows.forEach((row, i) => {
      const rowNum = i + 5
      const rowTotal = cols.reduce((a, c) => a + (row.valores[c.id] || 0), 0)
      ws2.getCell(rowNum, 1).value = i + 1
      ws2.getCell(rowNum, 1).border = allBorders
      ...
      totalCell.value = rowTotal > 0 ? rowTotal : null
    ...
    genCell.border = allBorders
    genCell.alignment = { horizontal: 'right' }

    // Fila total general label
    ...
    ws2.getCell(genLabelRow, 1).border = allBorders
    ws2.getCell(genLabelRow, 1).alignment = { horizontal: 'right' }
    const genValCell = ws2.getCell(genLabelRow, cols.length + 2)
    genValCell.value = general
    genValCell.numFmt = '0,##0'
    ...
    genValCell.border = allBorders
    genValCell.alignment = { horizontal: 'right' }
    ...
    hdrStyle(ws3.getCell('A1'), AZUL_OSC)
    ws3.getCell('A1').value = 'DISTRIBUCION DE INGRESOS'
    ws3.getCell('A1').font = { bold: true, color: { argb: BLANCO }, size: 13 }
    ws3.getRow(1).height = 28

    // Fecha
    ws3.mergeCells('A2:D2')
    ws3.getCell('A2').value = 'Fecha del registro: ' + fechaStr
    ws3.getCell('A2').font = { size: 10 }
    ws3.getCell('A2').border = allBorders

    // Total general
    ws3.mergeCells('A3:B3')
    ws3.getCell('A3').value = 'Total General (Caja 1105-05):'
    ws3.getCell('A3').font = { bold: true, size: 11 }
    ws3.getCell('A3').border = allBorders
    ws3.mergeCells('C3:D3')
    ws3.getCell('C3').value = general
    ws3.getCell('C3').numFmt = '0,##0'
    ...
    ws3.getCell('C3').border = allBorders
    ws3.getCell('C3').alignment = { horizontal: 'right' }

    // Encabezados
    ws3.getRow(4).height = 22
    ;['Concepto', 'Porcentaje', 'Monto Calculado', 'Monto Aproximado'].forEach((h, i) => {
      hdrStyle(ws3.getCell(4, i + 1), AZUL_CLARO, AZUL_OSC)
      ws3.getCell(4, i + 1).value = h
    })
    ...
    ws3.mergeCells(tdRow, 1, tdRow, 3)
    ws3.getCell(tdRow, 1).value = 'Total Distribucion'
    ws3.getCell(tdRow, 1).font = { bold: true, size: 10 }
    ws3.getCell(tdRow, 1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: GRIS } }
    ws3.getCell(tdRow, 1).border = allBorders
    ws3.getCell(tdRow, 1).alignment = { horizontal: 'right' }
    ws3.getCell(tdRow, 3).border = allBorders
    ws3.getCell(tdRow, 4).value = totalDist
    ws3.getCell(tdRow, 4).numFmt = '0,##0'
    ws3.getCell(tdRow, 4).font = { bold: true, size: 10 }
    ws3.getCell(tdRow, 4).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: GRIS } }
    ws3.getCell(tdRow, 4).border = allBorders
    ws3.getCell(tdRow, 4).alignment = { horizontal: 'right' }

    // Saldo restante
    const srRow = tdRow + 1
    ws3.mergeCells(srRow, 1, srRow, 3)
    const saldoColor = saldo >= 0 ? 'FF1A7A4A' : 'FFC0392B'
    const saldoBg    = saldo >= 0 ? 'FFE8F8F1' : 'FFFEE8E8'
    ws3.getCell(srRow, 1).value = 'Saldo Restante'
    ...
    ws3.getCell(srRow, 1).border = allBorders
    ws3.getCell(srRow, 1).alignment = { horizontal: 'right' }
    ws3.getCell(srRow, 3).border = allBorders
    ws3.getCell(srRow, 4).value = saldo
    ws3.getCell(srRow, 4).numFmt = '0,##0'
    ...
    ws3.getCell(srRow, 4).border = allBorders
    ws3.getCell(srRow, 4).alignment = { horizontal: 'right' }
    ...
    a.href = url
    a.download = 'Ingreso_' + fecha + '.xlsx'
    a.click()
